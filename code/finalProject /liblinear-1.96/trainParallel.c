...
void setparameter( struct parameter & param)
{
	...
	param.solver_type = my_slover_type;
	...
	//bias = -1;
	if(param.solver_type==PERCEPTRON || param.solver_type==LMS){
		prob.bias=1;
		bias=1;
	}
...
int main(int argc,char ** argv){
	// trainParallel nump numn train train trian.log type;
	int mpid, all;
	...
	ofstream fout;
	if(argc>5)
		fout.open(argv[5],std::ofstream::out | std::ofstream::app);
	else
		fout.open("timeRecord.log",std::ofstream::out | std::ofstream::app);
	my_slover_type=L2R_L2LOSS_SVC_DUAL;
	if(argc>=7)
		my_slover_type=atoi(argv[6]);
	MPI_Init(&argc, &argv);
	...
	fout<<"process:"<<mpid<<" use time "<<timeuse<<" ms."<<endl;

	if(mpid != 0)
		{
			message = 1;
			MPI_Send(&message,1,MPI_DOUBLE,0,12,MPI_COMM_WORLD);
	...
	else
	{
		...
		for(int q= 1;q<all;q++)
		{
			MPI_Recv(&message,1,MPI_DOUBLE,q,12,MPI_COMM_WORLD,&status);
		}
		gettimeofday(&endtime,0);
		double timeuse = 1000000*(endtime.tv_sec - starttime.tv_sec) + endtime.tv_usec - starttime.tv_usec;
		timeuse /=1000;
		fout<<"all process use time "<<timeuse<<" ms."<<endl;
	}
	MPI_Finalize();
	return 0;
}